/* __mshare_va_v3_1__ */
(function(){ if(window.__mshareV3_1) return; window.__mshareV3_1=true;
  var KEYS={pos:"va_pos_v7", hidden:"va_hidden_v3_1", muted:"va_muted_v3_1"};
  ...
  function chooseVoice(){
    ...
    var v = voices.find(function(v){return /Daniel/i.test(v.name)}) || voices.find(function(v){return /^en-GB/i.test(v.lang)}) || voices.find(function(v){return /^en/i.test(v.lang)});
    if(v){ VA.voiceName=v.name; VA.voiceLang=v.lang; }
    var sel=q("#mshare-voice-voice"); if(sel){
      var opt=[].slice.call(sel.options).find(function(o){return /Daniel\b/.test(o.text)||/\(en-GB\)/.test(o.text)});
      if(opt) sel.value=opt.value;
    }
  }
  if(window.speechSynthesis){ chooseVoice(); window.speechSynthesis.onvoiceschanged = chooseVoice; }
  function utter(text){ var u=new SpeechSynthesisUtterance(text); u.lang=VA.voiceLang||"en-GB"; u.voice=(window.speechSynthesis.getVoices&&window.speechSynthesis.getVoices().find(function(v){return v.name===VA.voiceName}))||null; u.rate=VA.rate; u.pitch=VA.pitch; return u; }
  ...
  /* ---- Toggle inside panel and floating button ---- */
  ...
  function ensureInternalHide(panel){ var b=panel.querySelector("#vaHideShow"); if(!b){ b=document.createElement("button"); b.id="vaHideShow"; b.type="button"; b.className="mshare-voicebot__btn"; b.textContent="Hide"; var handle=panel.querySelector(".mshare-voicebot__handle")||panel.firstElementChild||panel; handle.appendChild(b);} return b; }
  /* ---- Positioning: default at Voice button ---- */
  function defaultAtToggle(panel, toggle){ panel.classList.add("va-docked"); var tr=R(toggle); panel.hidden=false; panel.style.visibility="hidden"; requestAnimationFrame(function(){ var pr=R(panel); var left = tr.right - pr.width; var top = tr.bottom - pr.height; if(left<8) left=8; if(left>innerWidth-pr.width-8) left=innerWidth-pr.width-8; var hb=header()? (R(header()).bottom + window.scrollY) : MIN_TOP; var minTop=Math.max(MIN_TOP, hb - window.scrollY + 8); if(top<minTop) top=minTop; if(top>innerHeight-pr.height-16) top=innerHeight-pr.height-16; panel.style.left=left+"px"; panel.style.top=top+"px"; panel.style.right="auto"; panel.style.bottom="auto"; panel.style.visibility="visible"; }); }
  ...
  function clamp(panel){ var r=R(panel); if(!r) return; var nx=Math.min(Math.max(8,r.left), innerWidth-r.width-8); var hb=header()? (R(header()).bottom + window.scrollY) : MIN_TOP; var minTop=Math.max(MIN_TOP, hb - window.scrollY + 8); var ny=Math.min(Math.max(minTop,r.top), innerHeight-r.height-16); panel.style.left=nx+"px"; panel.style.top=ny+"px"; }
  ...
  /* ---- Drag (pointer events) ---- */
  function makeDraggable(panel){ var handle=panel.querySelector(".mshare-voicebot__handle")||panel; handle.style.touchAction="none"; var drag=false,sx=0,sy=0,ox=0,oy=0; handle.addEventListener("pointerdown",function(e){ handle.setPointerCapture&&handle.setPointerCapture(e.pointerId); var r=R(panel); ox=r.left; oy=r.top; sx=e.clientX; sy=e.clientY; drag=true; }); handle.addEventListener("pointermove",function(e){ if(!drag) return; panel.style.left=(ox+(e.clientX-sx))+"px"; panel.style.top=(oy+(e.clientY-sy))+"px"; panel.style.right="auto"; panel.style.bottom="auto"; clamp(panel); }); function up(e){ if(!drag) return; drag=false; var pr=R(panel), hr=R(header()), br=R(burger()); if(overlap(pr,hr)||overlap(pr,br) || pr.top<(MIN_TOP-8)){ var t=ensureToggle(); safeDock(panel,t,{respectSaved:false}); try{localStorage.removeItem(KEYS.pos);}catch(x){} } else { try{localStorage.setItem(KEYS.pos, JSON.stringify({left:pr.left, top:pr.top}));}catch(x){} panel.classList.remove("va-docked"); } handle.releasePointerCapture&&handle.releasePointerCapture(e.pointerId); } handle.addEventListener("pointerup",up); handle.addEventListener("pointercancel",up); }
  /* ---- Show/Hide + muted state ---- */
  ...
  function wireVAControls(panel){
    ...
    startBtn && startBtn.addEventListener("click", function(){ unlockAudio(); VA.muted=false; try{localStorage.setItem(KEYS.muted,"0");}catch(e){} if(VA.synth&&VA.synth.paused){ try{VA.synth.resume();}catch(e){} } });
    resumeBtn && resumeBtn.addEventListener("click", function(){ unlockAudio(); if(!VA.muted && VA.synth&&VA.synth.paused){ try{VA.synth.resume();}catch(e){} } });
    pauseBtn && pauseBtn.addEventListener("click", function(){ try{VA.synth&&VA.synth.pause();}catch(e){} });
  ...
  /* ---- Page Start/Stop sync (document delegation) ---- */
  function isStartLabel(t){ return /^\s*start\s*$/i.test(t); }
  function isStopLabel(t){ return /^\s*stop\s*$/i.test(t); }
  document.addEventListener("click", function(ev){
    var btn = ev.target && ev.target.closest && ev.target.closest("button");
    if(!btn) return;
    if(insidePanel(btn)) return; /* ignore VA own buttons */
    var label = (btn.textContent||"").trim();
    if(isStartLabel(label)){ VA.pageActive=true; VA.muted=true; try{localStorage.setItem(KEYS.muted,"1");}catch(e){} stopSpeak(); document.dispatchEvent(new CustomEvent("mshare:page-start",{bubbles:true})); }
    else if(isStopLabel(label)){ VA.pageActive=false; VA.muted=true; try{localStorage.setItem(KEYS.muted,"1");}catch(e){} stopSpeak(); document.dispatchEvent(new CustomEvent("mshare:page-stop",{bubbles:true})); }
  }, true);
  /* ---- Init wiring ---- */
  function wire(panel){ panel.style.removeProperty("inset"); panel.style.zIndex="9998";
    ...
    makeDraggable(panel); wireVAControls(panel);
    /* Daniel default selection */ chooseVoice();
    window.addEventListener("resize", function(){ safeDock(panel,toggle,{respectSaved:true}); });
    window.addEventListener("orientationchange", function(){ safeDock(panel,toggle,{respectSaved:true}); });
    ...
    /* place panel at button initially or restore if safe */
  ...
  function init(){ var p=findPanel(); if(!p) return; if(p.__vaV3_1) return; p.__vaV3_1=true; wire(p); }
