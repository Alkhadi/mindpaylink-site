/* __mshare_va_v3_2__ */
(function(){ if(window.__mshareV3_2) return; window.__mshareV3_2=true;
  var KEYS={pos:"va_pos_v8", hidden:"va_hidden_v3_2", muted:"va_muted_v3_2"};
  var MIN_TOP=96, THR=100;
  ...
  function utter(text){ var u=new SpeechSynthesisUtterance(text); u.lang=VA.voiceLang||"en-GB";
    try{ var vs=window.speechSynthesis.getVoices&&window.speechSynthesis.getVoices(); u.voice = vs && vs.find(function(v){return v.name===VA.voiceName}) || null; }catch(e){}
    u.rate=VA.rate; u.pitch=VA.pitch; return u; }
  ...
  /* ---- Docking (panel opens exactly at Voice button) ---- */
  ...
  /* ---- Accurate drag (pointer; clamped; persisted) ---- */
  function makeDraggable(panel){ var handle=panel.querySelector(".mshare-voicebot__handle")||panel; handle.style.touchAction="none"; var drag=false,sx=0,sy=0,ox=0,oy=0; handle.addEventListener("pointerdown",function(e){ handle.setPointerCapture&&handle.setPointerCapture(e.pointerId); var r=R(panel); ox=r.left; oy=r.top; sx=e.clientX; sy=e.clientY; drag=true; }); handle.addEventListener("pointermove",function(e){ if(!drag) return; var nx=ox+(e.clientX-sx), ny=oy+(e.clientY-sy); panel.style.left=nx+"px"; panel.style.top=ny+"px"; panel.style.right="auto"; panel.style.bottom="auto"; clamp(panel); }); function up(e){ if(!drag) return; drag=false; var pr=R(panel), hr=R(header()), br=R(burger()); if(overlap(pr,hr)||overlap(pr,br) || pr.top<(MIN_TOP-8)){ var t=ensureToggle(); safeDock(panel,t,{respectSaved:false}); try{localStorage.removeItem(KEYS.pos);}catch(x){} } else { try{localStorage.setItem(KEYS.pos, JSON.stringify({left:pr.left, top:pr.top}));}catch(x){} panel.classList.remove("va-docked"); } handle.releasePointerCapture&&handle.releasePointerCapture(e.pointerId); } handle.addEventListener("pointerup",up); handle.addEventListener("pointercancel",up); }
  /* ---- Show/Hide + muted-by-default ---- */
  ...
  /* ---- Page Start/Stop hard-sync (buttons on every page) ---- */
  ...
  document.addEventListener("click", function(ev){
    var btn = ev.target && ev.target.closest && ev.target.closest("button, [role=button], .btn"); if(!btn) return;
    if(insidePanel(btn)) return; /* ignore VA internal buttons */
  ...
  function wire(panel){ panel.style.removeProperty("inset"); panel.style.zIndex="9998";
    ...
    window.mshareVA = { cancel: stopSpeak, muted: ()=>VA.muted, setMuted:(m)=>{VA.muted=!!m; try{localStorage.setItem(KEYS.muted,m?"1":"0");}catch(e){}}, chooseVoice: chooseVoice };
  }
  function init(){ var p=findPanel(); if(!p) return; if(p.__vaV3_2) return; p.__vaV3_2=true; wire(p); }
