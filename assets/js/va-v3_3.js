/* __mshare_va_v3_3__ */
(function(){ if(window.__mshareV3_3) return; window.__mshareV3_3=true;
  var KEYS={pos:"va_pos_v9", hidden:"va_hidden_v3_3", muted:"va_muted_v3_3"};
  ...
  /* ---- Precise drag (pointer; clamped; persisted) ---- */
  function makeDraggable(panel){ var handle=panel.querySelector(".mshare-voicebot__handle")||panel; handle.style.touchAction="none"; var drag=false,sx=0,sy=0,ox=0,oy=0; handle.addEventListener("pointerdown",function(e){ handle.setPointerCapture&&handle.setPointerCapture(e.pointerId); var r=R(panel); ox=r.left; oy=r.top; sx=e.clientX; sy=e.clientY; drag=true; }); handle.addEventListener("pointermove",function(e){ if(!drag) return; var nx=ox+(e.clientX-sx), ny=oy+(e.clientY-sy); panel.style.left=nx+"px"; panel.style.top=ny+"px"; panel.style.right="auto"; panel.style.bottom="auto"; clamp(panel); }); function up(e){ if(!drag) return; drag=false; var pr=R(panel), hr=R(header()), br=R(burger()); if(overlap(pr,hr)||overlap(pr,br) || pr.top<(96-8)){ var t=ensureToggle(); safeDock(panel,t,{respectSaved:false}); try{localStorage.removeItem(KEYS.pos);}catch(x){} } else { try{localStorage.setItem(KEYS.pos, JSON.stringify({left:pr.left, top:pr.top}));}catch(x){} panel.classList.remove("va-docked"); } handle.releasePointerCapture&&handle.releasePointerCapture(e.pointerId); } handle.addEventListener("pointerup",up); handle.addEventListener("pointercancel",up); }
  /* ---- Visibility & mute ---- */
  ...
  function init(){ var p=findPanel(); if(!p) return; if(p.__vaV3_3) return; p.__vaV3_3=true; wire(p); }
